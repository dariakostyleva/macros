...
void ana_si_detectors_real(){
   ...
   //   TH1F* h_s_pz = new TH1F("h_s_pz","Longitudinal momentum of HI (GeV/c)",500,35.6,36.2);
    ...
    Double_t x[2][4] = {{0}}, y[2][4]= {{0}}, z[2][4]= {{0}}; //1st index - particle (0 - 29S, 1 - proton), 2nd - detector number (there are 4 silicons)
    ...
    //for(Int_t i=0;i<nevents;i++){
       ...
       for(Int_t h=0;h<traPerEvent;h++){         
           ...
           if(Tra[h]->GetPdi()==2212 && Tra[h]->GetDetCopyID()==1){
              ...
              //printf("e_p_lab = %f\n",e_p_lab); 
              //cout << "Found proton in 1st detector with Eloss " << endl;
            }
